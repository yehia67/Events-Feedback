...
contract FeedBack {
  ...
  function createSession(string _sessionName,string _description,uint _feedbackTime,byte[] _lecturer,address[] _attendes) public
  {
  ...
  // Make feedback
   ...
   function initFeedback(string _sessionName) private{
       feedback[_sessionName] = new uint8[](11);
    }
     function feedBack(string _sessionNam,uint _feedback) private{
  ...
  function Time(string _sessionName) public view returns (bool){
       return (now >=   sessions[_sessionName].feedbackDate );          
  }
  function setFeedbackTime(string _sessionName) private{
  ...
  function findAddress(address _attende,address[] _attendes)  public pure returns (bool) {
  ...
  function validate(string _sessionName,address[] _attendes) private view returns(bool){
            //Attende
  ...
  //Take vote
  function takeVote(string _sessionName,uint _feedback)public{
  ...
  //See result
  function seeResult(string _sessionName) public view returns(uint8[]){
